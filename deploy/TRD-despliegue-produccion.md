...
| 3 | `nginx.conf.example` cortaba subidas en 50MB; el límite real de la app (`/api/carga`) es 55MB — un Excel de 51-55MB pasaba la validación de la app pero nunca llegaba, rechazado por Nginx antes. | `client_max_body_size` subido a 60MB (con margen). |
