...
import { getRepuestosMetricsAction, getRepuestosTrendAction } from "@/lib/actions/repuestos";
...
export default function RepuestosPage() {
  ...
  const { data: metricsData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const data = await getRepuestosMetricsAction({ ranges: dateRanges });
      return { facturas: data };
    },
  });

  const { data: trendData } = useQuery({
    queryKey: ["repuestos-trend", anio, JSON.stringify(meses)],
    queryFn: async () => {
      const data = await getRepuestosTrendAction({ anio });
  ...
  const porSucursal = useMemo(() => {
    ...
    metricsData.facturas.forEach((r) => {
      if (r.sucursalId && map.has(r.sucursalId)) {
        map.get(r.sucursalId)!.monto += Number(r.monto ?? 0);
      }
    });
  ...
  const topClientes = useMemo(() => {
    ...
    metricsData?.facturas.forEach((r) => {
      ...
      map.get(k)!.monto += Number(r.monto ?? 0);
    });
    return Array.from(map.values())
      .sort((a, b) => b.monto - a.monto)
      .slice(0, 10);
