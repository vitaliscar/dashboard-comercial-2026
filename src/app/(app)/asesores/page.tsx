...
export default function AsesoresPage() {
  ...
  const drilldownData = useMemo(() => {
    if (!selectedAdvisor || !rawDrilldown) return null;

    const aliases = new Map<string, string>();
    rawDrilldown.aliases.forEach((row) => {
    ...
    const filteredMetas = rawDrilldown.metas.filter((r) => filterByAdvisor(r, "codigo"));
    const filteredFacturas = rawDrilldown.facturas.filter((r) => filterByAdvisor(r, "nombre"));
    const filteredCotizaciones = rawDrilldown.cotizaciones.filter((r) =>
      filterByAdvisor(r, "codigo"),
    );
    const filteredPerdidas = rawDrilldown.perdidas.filter((r) => filterByAdvisor(r, "nombre"));
                          ...
                          {drilldownData.recentCotizaciones.map((c, i) => (
                            ...
                            >
                              ...
                              <div className="text-right shrink-0">
                                <p className="font-bold">{money(Number(c.monto))}</p>
                        ...
                        <div className="flex flex-col gap-2 max-h-56 overflow-y-auto pr-1">
                          {drilldownData.recentPerdidas.map((p, i) => (
                            ...
                            >
                              ...
                              <div className="text-right shrink-0">
                                <p className="font-bold">{money(Number(p.monto))}</p>
