...
import { getDateRangesForMonths, applyMonthFilterToQuery } from "@/lib/date-range";
...
export const Route = createFileRoute("/_app/alquiler")({
  head: () => ({ meta: [{ title: "Dashboard Alquiler · CCV" }] }),
  component: Alquiler,
});
...
function Alquiler() {
  ...
  // Fetch facturación data for Alquiler
  const { data: facturacionMensual } = useQuery({
    queryKey: ["alquiler-facturacion", anio, unitId, role, profile?.id],
    queryFn: async () => {
      let q = supabase
        .from("facturas")
        .select("monto, fecha")
  ...
  const { data: presupuestoData } = useQuery({
    queryKey: ["alquiler-presupuesto", anio, JSON.stringify(meses), unitId, role, profile?.id],
    queryFn: async () => {
  ...
  const { data: clientesCobro } = useQuery({
    queryKey: ["alquiler-clientes-cobro", unitId, role, profile?.id],
    queryFn: async () => {
