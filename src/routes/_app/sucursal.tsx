...
export const Route = createFileRoute("/_app/sucursal")({
  head: () => ({ meta: [{ title: "Mi Sucursal · CCV" }] }),
...
function SucursalPage() {
  ...
  const { data: metrics, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let pq = scoped(
        supabase.from("presupuestos").select("*").eq("anio", anio),
        role,
        profile,
        profile?.id,
      ...
      if (selectedSucursales.length > 0) {
        fq = fq.in("sucursal_id", selectedSucursales);
        vq = vq.in("sucursal_id", selectedSucursales);
        pq = pq.in("sucursal_id", selectedSucursales);
      }
      ...
      const [f, v, p] = await Promise.all([fq, vq, pq]);
      return {
        facturacion: f.data ?? [],
        perdidas: v.data ?? [],
        presupuestos: p.data ?? [],
  ...
  const { data: trend } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (selectedSucursales.length > 0) fq = fq.in("sucursal_id", selectedSucursales);
      if (selectedUnidades.length > 0) fq = fq.in("unidad_negocio_id", selectedUnidades);

      let pq = scoped(
        supabase.from("presupuestos").select("monto, mes").eq("anio", anio),
        role,
        profile,
        profile?.id,
        { sucursal: "sucursal_id", unidad: "unidad_negocio_id" },
      ...
      pq = applyMonthFilterToQuery(pq, meses, anio);
      if (selectedSucursales.length > 0) pq = pq.in("sucursal_id", selectedSucursales);
