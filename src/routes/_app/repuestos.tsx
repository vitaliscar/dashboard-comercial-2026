...
// Repuestos unit ID (canonical from DB)
...
function Repuestos() {
  ...
  // GC can only see their own unit; for others lock to Repuestos unit
  const isGC = role === "gerente_comercial";
  // For gerencia we show all Repuestos; for GC we use their unidades_negocio_ids (which should include Repuestos)
  const unitId = REPUESTOS_UNIT_ID;
  ...
  const { data: trendData } = useQuery({
    queryKey: ["repuestos-trend", anio, JSON.stringify(meses), role, profile?.id],
    queryFn: async () => {
      let fq = supabase
        .from("facturas")
        .select("monto, fecha")
