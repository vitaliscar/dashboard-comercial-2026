...
export const Route = createFileRoute("/_app/equipos")({
  head: () => ({ meta: [{ title: "Dashboard Equipos · CCV" }] }),
  component: Equipos,
});
...
function Equipos() {
  ...
  const unitId = isGC && profile?.unidad_negocio_id ? profile.unidad_negocio_id : "all";
  ...
  // Fetch real data from facturas, presupuestos, ventas_perdidas
  const { data: facturacionMensual } = useQuery({
    queryKey: ["equipos-facturacion", anio, unitId, role, profile?.id],
    queryFn: async () => {
      let q = supabase
        .from("facturas")
        .select("monto, fecha")
      ...
      if (unitId !== "all") q = q.eq("unidad_negocio_id", unitId);
  ...
  const { data: presupuestoData } = useQuery({
    queryKey: ["equipos-presupuesto", anio, JSON.stringify(meses), unitId, role, profile?.id],
    queryFn: async () => {
      let q = supabase.from("presupuestos").select("monto").eq("anio", anio);
  ...
  const { data: ventasPerdidas } = useQuery({
    queryKey: ["equipos-ventas-perdidas", anio, JSON.stringify(meses), unitId, role, profile?.id],
    queryFn: async () => {
      let q = supabase.from("ventas_perdidas").select("*");
  ...
  const { data: clientesCobro } = useQuery({
    queryKey: ["equipos-clientes-cobro", unitId, role, profile?.id],
    queryFn: async () => {
      let q = supabase.from("cobranzas").select("*").gt("saldo", 0);
      if (unitId !== "all") q = q.eq("unidad_negocio_id", unitId);
  ...
  const clientesFiltrados = useMemo(() => {
    ...
    return (
      (clientesCobro ?? [])
        ...
        .sort((a: any, b: any) => Number(b.saldo) - Number(a.saldo))
    );
  ...
  return (
    <div className="flex flex-col gap-6 max-w-400">
      ...
      <FilterHeader
        ...
        unitOptions={role === "gerencia" ? undefined : undefined}
        defaultMes={meses}
        defaultAnio={anio}
        showAllMonths
      />
      ...
      {/* Charts row 2 - Cumplimiento y Ventas Perdidas detalle */}
