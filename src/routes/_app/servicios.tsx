...
import { Zap, Wrench, User, TrendingUp, Building2, Search } from "lucide-react";
...
function Servicios() {
  ...
  // Lock unit for GC and sucursal for Coordinador; only Gerencia can change them
  ...
  const { data: servicios, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (sucursal !== "all") q = q.eq("sucursal_id", sucursal);
      if (unidadesSeleccionadas.length > 0) q = q.in("unidad_negocio_id", unidadesSeleccionadas);
  ...
  // Fetch cobranzas for table
  const { data: cobranzasData } = useQuery({
    queryKey: ["cobranzas-servicios", filterKey, role, profile?.id],
    queryFn: async () => {
      let q = supabase.from("cobranzas").select("*").gt("saldo", 0).order("fecha_vencimiento");
  ...
  const totales = useMemo(() => {
    ...
    servicios.forEach((s) => {
      const cat = (s.categoria_venta ?? "").toLowerCase();
  ...
  const cumplimientoSucursal = useMemo(() => {
    if (!servicios || !sucursales) return [];
    // For now, calculate as count-based. In a real scenario, compare against presupuesto
    ...
    (servicios ?? []).forEach((r) => {
      if (r.sucursal_id && map.has(r.sucursal_id)) {
        const entry = map.get(r.sucursal_id)!;
        entry.count += 1;
  ...
  return (
    <div className="flex flex-col gap-6 max-w-400">
      ...
      {/* Facturación por Sucursal (barras) */}
      <div className="card-elevated p-5">
        <div className="mb-4">
      ...
      {/* Grid de 3 gráficos */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Participación ventas 2026 (donut) */}
        <div className="card-elevated p-5">
        ...
        {/* Porcentaje cumplimiento por sucursal */}
        <div className="card-elevated p-5">
