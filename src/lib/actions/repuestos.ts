...
export async function getPresupuestosRepuestosAction(data: {
  anio: number;
  meses: MonthFilter;
  sucursal: string | "all";
}) {
    ...
    return tx
      ...
      .where(
        and(
          ...
          eq(presupuestos.unidadNegocioId, await unidadId("repuestos")),
        ),
      );
  });
}
...
// Detalle de ventas por marca (Caterpillar, Blumaq, Vms Corporation, etc.) x
// mes — hoja "Detalles de Ventas Repuestos", sin filtro de sucursal (el Excel
// fuente no la trae a este nivel).
export async function getDetallesVentasRepuestosAction(data: { meses: MonthFilter }) {
  return withAuth(async ({ tx }) => {
    ...
    return tx
      ...
      .where(
        !isAllMonths && monthList.length > 0
          ? inArray(detallesVentasRepuestos.mes, monthList)
          : undefined,
      );
  });
}
